...
enum {
    ...
    STATE_HOLDING,
    // This state is set while calling `process_record()`, which will
    // recursively
    // call `process_achordion()`. This state is checked so that we don't
    // process
...
bool process_achordion(uint16_t keycode, keyrecord_t* record) {
    ...
    // this information is saved to a flag to be processed later when the
        ...
        if (is_tap_hold && record->tap.count == 0 && record->event.pressed &&
            ...
            if (timeout > 0) {
                ...
                if (is_mt) { // Apply mods immediately if they are "eager."
                    uint8_t mod =
                        mod_config(QK_MOD_TAP_GET_MODS(tap_hold_keycode));
...
#    endif
    ...
    if (keycode == tap_hold_keycode && !record->event.pressed) {
        ...
        if (achordion_state == STATE_HOLDING) {
            ...
            // No other key was pressed between the press and release of the
            // tap-hold key, simulate a hold and then a release without waiting
            // for Achordion timeout to end.
...
#    endif
        ...
        //
        // Otherwise, we call `achordion_chord()` to determine whether to settle
        // the tap-hold key as tapped vs. held. We implement the tap or hold by
        // plumbing events back into the handling pipeline so that QMK features
        // and other user code can see them. This is done by calling
        // `process_record()`, which in turn calls most handlers including
        // `process_record_user()`.
        if (!is_streak &&
...
#    endif // TAP_CODE_DELAY > 0
        ...
        recursively_process_record(record,
                                   achordion_state); // Re-process event.
...
#    ifdef ACHORDION_STREAK
__attribute__((weak)) uint16_t achordion_streak_timeout(
